...
void ConsultarEstadisticas(){
    ...
    switch(opcion){
        ...
        case 2:
            ...
            std::cout << "\n";
            nickname = pedirString("el nickname del profesor al que quiere consultar sus estadisticas");
            profe = ControladorU->ObtenerProfesor(nickname);
            if(!profe){
                std::cout << "Profesor ingresado no existe" << std::endl;
                pausa("Presione enter para continuar...");
                return;
            }
            valido = ControladorU->ExisteProfesor(nickname);
            if(valido != true){
                std::cout << "No existe profesor con " << nickname << "en el sistema" << std::endl;
                return;
            }
            estadisticascurso = ControladorU->consultarEstadisticasProfesor(nickname);
            if (estadisticascurso != nullptr) {
                itpp = estadisticascurso->getIterator();
                while (itpp->hasCurrent()) {
                    DataEstadisticaProfesor* data = dynamic_cast<DataEstadisticaProfesor*>(itpp->getCurrent());
                    if (data != nullptr) {
                        std::cout << std::endl;
                        std::cout << "Nombre del curso: " << data->getNombreCurso() << std::endl;
                        std::cout << "Descripcion del curso: " << data->getDescripcion() << std::endl;
                        std::cout << "Dificultad del curso: " << obtenerTipoDificultad(data->getDificultad()) << std::endl;
                        std::cout << "Promedio de avance: " << data->getPromedioAvance() << "%" << std::endl;
                    }
                    itpp->next();
                }
                delete itpp;
            } else {
                std::cout << "No se encontraron estadísticas para el profesor." << std::endl;
            }
            break;
