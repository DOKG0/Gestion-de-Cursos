...
DataEstadisticaCurso* ControladorCurso::consultarEstadisticasCurso(std::string nombreCurso) {
    ...
    // Obtener el diccionario de registros de inscripción al curso
    IDictionary* registros = curso->getRegistroInscripcion();
    if (registros == nullptr || registros->isEmpty()) {
        std::cout << "No hay registros de inscripción para este curso" << std::endl;
        return new DataEstadisticaCurso(nombreCurso, 0.0, curso->getDescripcionCurso(), curso->getDificultad());
    }

    std::cout << "Cantidad de registros de inscripción: " << registros->getSize() << std::endl;
    std::cout << std::endl;
    ...
    while (it->hasCurrent()) {
        ...
        if (registro != nullptr) {
            double porcentajeAprobacion = registro->getPorcentajeAprobacion();
            
            // Imprimir para depuración
            std::cout << "Estudiante: " << registro->getEstudiante()->getNickname() << " - Porcentaje de aprobación: " << porcentajeAprobacion << std::endl;
            
            sumaPorcentajes += porcentajeAprobacion;
            cantidadEstudiantes++;
        } else {
            std::cout << "Registro no válido en los registros de inscripción." << std::endl;
        }
        it->next();
    }
    delete it;
